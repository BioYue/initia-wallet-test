...
export function uploadSignature(
    ...
    const msg = new MsgExecute(sender, contractAddress, module, functionName, typeArgs, args);
 ...
 */
export function sendToken(sender: string, contractAddress: string, amount: string) {
    const sendMsg = new MsgSend(sender, contractAddress, amount);

    return sendMsg;
}

export function bridgeToken(sender: string, bridgeId: number, to: string, amount: Coin) {
    const msg = new MsgInitiateTokenDeposit(sender, bridgeId, to, amount);

    return msg;
}

export function bridgeOutToken(sender: string, to: string, amount: Coin) {
    const msg = new MsgInitiateTokenWithdrawal(sender, to, amount);
